...
class MtList<T> implements IList<T> { //doesn’t make any changes to the empty list
...
return ; }
  ...
  //  Fill in each blank below so that the associated test passes (Note: merely copying the test expression or partially evaluating it is not a sufficient answer!). If a given test will always fail, instead write BROKEN in that test’s blank, and to the side of it briefly explain your reasoning.
class ExamplesIList {
  IList<Integer> mt = new MtList<Integer>();
  IList<Integer> ints1 = new ConsList<Integer>(1, mt);
  IList<Integer> ints2 = new ConsList<Integer>(2, new
      ConsList<Integer>(3, mt));
  IList<Integer> ints3 = new ConsList<Integer>(4, new
      ConsList<Integer>(5, mt));
  IList<Integer> ints4 = new ConsList<Integer>(6, new
      ConsList<Integer>(7, mt));
// Fill in the
  ...
  void testAppend(Tester t) {
    ...
    ints4.append(ints4);
   // t.checkExpect(ints4.length(), _________________);
    }
}